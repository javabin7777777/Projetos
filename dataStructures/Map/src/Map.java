   ...
   public static void remove() {
      if(map.isEmpty()) System.out.println("\n Map is Empty");
         ...
         for(Entry<String, Integer> kv: set) {
            if(kv.getKey().equalsIgnoreCase(name)) map.remove(kv.getKey(), kv.getValue());
            find=true;
         }
         if(find) System.out.printf("%n%S","removed");
         else System.out.printf(" %n%S","player not found");
         find=false;
      }
   }
      ...
      if(map.isEmpty()) System.out.println("\n Map is Empty");
            ...
            while(true) {
               ...
               switch(String.valueOf(in)) {
                  ...
                  default:    System.out.println("Options 1 or 2 only.");
               }
               if(in==0) break;
         }
         if(find) System.out.printf(" %n%S","updated");
         else System.out.printf(" %n%S%n","player not found");
         find=false;
      }
   }
}
